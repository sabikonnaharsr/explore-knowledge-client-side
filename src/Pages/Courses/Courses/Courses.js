    ...
    <div>
      <section class="bg-[url(https://c1.wallpaperflare.com/preview/741/52/995/old-books-book-books-old.jpg)] bg-cover bg-center bg-no-repeat">
        <div className="flex flex-col  lg:grid lg:grid-cols-9">
          <div className="lg:grid lg:grid-cols-3 gap-16 col-span-7 p-12">
            {data.map((card) => (
              <CourseDetails key={card.uid} card={card}></CourseDetails>
            ))}
          </div>
          <div className="lg:grid lg:col-span-2  p-12 lg:p-0 ">
            <ul class="mt-4 space-y-2 bg-slate-800">
              {category.map((cd) => (
                <LeftSideNav key={cd.id} cd={cd}></LeftSideNav>
              ))}
            </ul>
          </div>
        </div>
